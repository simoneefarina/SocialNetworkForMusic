document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('removePlaylistForm').addEventListener('submit', function(event) {
        ...
        const removePlaylistModal = new bootstrap.Modal(document.getElementById('removePlaylistModal'));
...
async function removePlaylist(playlistId) {
    try {
        const response = await fetch(`/api/removeplaylist/${playlistId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
